...
import { ALL_POST_SLUGS_QUERY, POST_BY_SLUG_QUERY, POST_BY_SLUG_QUERY_NO_SEO } from "@/lib/wpQueries";
...
export const revalidate = 60; // ISR: 60s para detalle
          ...
          {post.categories?.nodes?.length ? (
            <div className="mt-2 flex flex-wrap gap-2">
              {post.categories.nodes.map((c) => (
                <span key={c.id} className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-700">
                  {c.name}
                </span>
              ))}
            </div>
